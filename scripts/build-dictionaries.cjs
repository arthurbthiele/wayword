...
const disconnectedValidContent =
  "// Generated by scripts/build-dictionaries.cjs — do not edit by hand.\n" +
  "// Words from the full dict B source that aren't in the playable\n" +
  "// wordGraph (either outside the 'a'-connected component, length-floored\n" +
  "// out, or otherwise unreachable). Used only for friendlier rejection\n" +
  "// messages: typing one still fails, just with 'valid word, but not in\n" +
  "// our dictionary' rather than 'not a word'. Excludes slurs.\n" +
  "export const disconnectedValidWords: ReadonlySet<string> = new Set([\n" +
  disconnectedValid.map((w) => `  ${JSON.stringify(w)},`).join("\n") +
  "\n]);\n";
fs.writeFileSync(
  path.join(outDir, "disconnectedValidWords.ts"),
  disconnectedValidContent
);
...
console.log(`  src/dictionaryData/disconnectedValidWords.ts   (${bytes("disconnectedValidWords.ts")} bytes, ${disconnectedValid.length} words)`);
